...
| Provider 路由 | `~/.opencodex/providers.json` | 来自 AITabby/opencodex 的做法；记录 provider name → base_url / wire_api / env_key |
| 自定义模型目录 | `~/.opencodex/custom_model_catalog.json` | 用户在 Codex Box 里"加模型"最终落在这里；拼装进 `~/.codex/config.toml` 的 `[model_providers.*]` |
...
- Codex Box **复用** AITabby/opencodex 在 `~/.opencodex/` 下的两个 JSON 路径约定，让用户可以同时使用 AITabby/opencodex 的 CLI 工具与 Codex Box。
...
- 新定义：**"BYOK 模型目录与多 provider 路由底座"** —— 包括 `ModelCatalogEntry` 数据模型、`ProviderRoute` 数据模型、`CodexConfigChange` 写入操作、`~/.opencodex/` JSON 读写、Codex Desktop 安装检测（只读）。
...
- 与 AITabby/opencodex CLI 工具互不冲突，可共存。
...
- AITabby/opencodex 自身 schema 可能升级（providers.json / custom_model_catalog.json 字段变化）；Codex Box 需要在 `~/.opencodex/` 读写时做"未知字段保留"。
