...
在 Codex Box 的 BYOK 设计里，`Provider` 一词被用于多个层级：Codex 原生 `model_provider`、`[model_providers.*]`、`~/.opencodex/providers.json`、`custom_model_catalog.json` 中的 `provider/backend_provider`。这些概念如果不拆开，会导致 UI 和写入逻辑混乱：用户无法判断“添加模型来源”“Provider 路由”“工作配置”“Codex Runtime 启用”分别影响哪一层。
...
- 默认不强制改成 `openai` 或 `opencodex`。
- 用户选择 `openai`、`opencodex`、`codex_local_access`、其它自定义 provider 都应被视为合法路径，只要对应配置可解析。
...
对应 `~/.opencodex/custom_model_catalog.json` 中每个模型条目：
...
- 添加模型时，用户应能选择/编辑展示归属 provider，而不是写死为 `openai` 或 `opencodex`。
...
模型目录: ~/.opencodex/custom_model_catalog.json
当前模型: minimax
